class TokenMailer < BaseMailer
  ...
  def support_email(email, support)
    @support = support
    mail to: email, subject:"[Coinchel] Email from user - #{Time.zone.now.strftime("%Y-%m-%d %I:%M:%S")}(UTC)"
  end

  def activation(email, token)
    @token_url = edit_activation_url token
    mail to: email, subject:"[Coinchel] Confirm Your Registration - #{Time.zone.now.strftime("%Y-%m-%d %I:%M:%S")}(UTC)"
  end
  ...
  def send_token_on_mobile(email, token_code, ip)
    ...
    @email = email
    mail to: email, subject: "[Coinchel] Password Reset from #{ip} at - #{Time.zone.now.strftime("%Y-%m-%d %I:%M:%S")}(UTC)"
  end
  ...
  def signin_verify(email, ip_address, user_agent, token, location, is_devise = false)
     ...
     @location =location
     mail to: email, subject:  "[Coinchel] Authorize New Device #{@user_agent.browser},#{@user_agent.os},#{@user_agent.version} - #{@ip_address} at - #{Time.zone.now.strftime("%Y-%m-%d %I:%M:%S")}(UTC)"
  end
